...
from dos.components.fuse.extractor_sd import load_model, process_features_and_mask, get_mask
...
from dos.utils.utils_correspondence import pairwise_sim, draw_correspondences_gathered, chunk_cosine_sim, co_pca, find_nearest_patchs, \
                                draw_correspondences_lines, draw_correspondences_1_image, resize, draw_kp_on_image
...
sd_model, aug = load_model(config_path ='Panoptic/odise_label_coco_50e.py', diffusion_ver=VER, image_size=SIZE, num_timesteps=TIMESTEP, block_indices=tuple(INDICES))    

def compute_correspondences_sd_dino(img1, img1_kps, img2, index, model=sd_model, aug=aug, files=None, category='horse', mask=False, dist='l2', thresholds=None, real_size=960):  # kps,
    # print('compute_correspondences func is running...')
    
    img_size = 840 if DINOV2 else 224 if ONLY_DINO else 480    # ORIGINAL CODE # Ques: should it be 224 or 240, because 60 * stride(i.e 4) is 240
    ...
    img1_y, img1_x = img1_kps[:, 1].cpu(), img1_kps[:, 0].cpu()         # img1_kps should be [(num of kps)20,2]
    ...
    with torch.no_grad():
        if not CO_PCA:
            ...
            if not ONLY_DINO:
                ...
                features1 = process_features_and_mask(model, aug, img1_input, input_text=input_text,  mask=False, raw=True)  # features1 is a dict
            ...
            if FUSE_DINO:
                ...
                img1_batch = extractor.preprocess_pil(img1)                                             # img1_batch shape is torch.Size([1, 3, 840, 840])
    ...
    img2 = draw_kp_on_image(kps_1_to_2, img2) #, color = None)
    ...
    img2.save(f'img_pred_debug_{index}.png', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    img1_kps_for_rendered_image_1 = [[200.6771, 499.5944],
        [270.3177, 482.1484],
        [307.9398, 447.1939],
        [379.2488, 326.9812],
        [691.3879, 328.4179],
    ...
    img1_kps_for_cycle_correspond_1 = [[244.5000, 440.5000],   
        [314.5000, 552.5000],
        [370.5000, 440.5000],
        [384.5000, 384.5000],
        [706.5000, 314.5000],
    ...
    # kps correspond to img_pred_corres_NEW_1.png
    img1_kps_for_cycle_correspond_5 = [[202.5000, 566.5000],
        [286.5000, 496.5000],
        [258.5000, 426.5000],
        [356.5000, 314.5000],
        [692.5000, 384.5000],
    ...
    fig = draw_kp_on_image(img1_kps_for_rendered_image_1, rendered_image)
    fig.save(f'output_folder/rendered_image_1_with_kps.png', bbox_inches='tight')
    
    
    # For cycle correspondence
    fig = draw_kp_on_image(img1_kps_for_cycle_correspond_5, rendered_image)
    fig.save(f'output_folder/img1_kps_for_cycle_correspond.png', bbox_inches='tight')
    ...
    fig = draw_kp_on_image(img1_kps, rendered_image_updated)
    fig.save(f'output_folder/rendered_image_for_debug.png', bbox_inches='tight')
    print('fig size', fig)
    ...
    for index in range(len(img_files)):
        ...
        compute_correspondences_sd_dino(img1=rendered_image_updated, img1_kps=img1_kps, img2=target_image_updated, index=index, model = sd_model, aug = aug)